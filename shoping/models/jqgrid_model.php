...
class Jqgrid_model extends CI_Model {
	...
	*/
	...
	function set_data($table,$id_field,$where = FALSE,$prefix = FALSE, $extra = FALSE)
	{				
		$get_field = $this->db->list_fields($table);	
		foreach ($get_field as $field):
			if ($prefix):
		...
		if (is_array($where))
			foreach ($where as $field=>$value):
				unset($data[$field]); // flush the field form $_POST
			endforeach;
		...
		unset($data[$id_field]); // flush the id form $_POST
